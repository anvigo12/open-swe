...
describe("buildUsageDiagnostics", () => {
  it("carries only allowlisted displayed state", () => {
    ...
    const build = result.build as Record<string, Record<string, unknown>>
    expect(build.backend?.revision_id).toBe("rev-42")
  ...
  it("marks identifiers and times unknown when the backend cannot supply them", () => {
    const result = buildUsageDiagnostics({
      period: "7d",
      reports: [],
    ...
    expect(result.build).toBe("unavailable_from_backend")
    expect(result.api).toEqual({ origin: null, path: "/dashboard/api" })
